<template>
    ...
    >
        <!-- Discount Badge -->
        <span v-if="product.discount" class="bg-opacity-75 absolute top-2 right-2 rounded bg-secondary/75 px-2 py-1 text-xs font-bold text-white">
...
// Check if product is in wishlist (from global Inertia shared props)
const isInWishlist = computed(() => {
    const wishlistIds = page.props.auth?.wishlistProductIds ?? [];
