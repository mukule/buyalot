...
<template>
    ...
    >
        ...
        <Link :href="`/products/${encodeURIComponent(product.product_slug)}?v=${encodeURIComponent(product.id)}`" class="relative block flex-1">
            <div class="relative flex w-full justify-center overflow-hidden rounded-md bg-gray-100">
                <img
                    :src="product.image ?? ''"
                    :alt="product.name"
                    loading="lazy"
                    class="max-h-60 w-full object-contain opacity-0 transition-all duration-500 ease-in-out"
