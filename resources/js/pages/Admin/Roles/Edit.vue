...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div
      class="flex h-full flex-1 flex-col space-y-4 rounded-xl bg-white p-4 text-[color:var(--card-foreground)]"
    >
      <div class="flex items-center justify-between">
        <h4 class="text-2xl font-bold">{{ title }}</h4>
        <Link
          href="/admin/roles"
          class="text-sm text-[color:var(--primary)] hover:underline"
        >
          ← Back
        </Link>
      </div>
      ...
      <form @submit.prevent="submit" class="mt-2 space-y-4 px-4">
        ...
        <div>
          ...
          <div
            class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 max-h-72 overflow-auto rounded border border-[color:var(--border)] p-3"
          >
            <label
