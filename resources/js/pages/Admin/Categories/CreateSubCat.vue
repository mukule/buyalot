...
// Use your generic AppPageProps to include global props + category
const page = usePage<AppPageProps<{ category: Category }>>();
const category = page.props.category;

const title = `Create Subcategory for ${category.name}`;
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-4">
            <div class="flex h-full flex-1 flex-col space-y-4 rounded-xl bg-white p-4 text-[color:var(--card-foreground)]">
                ...
                <form
                    @submit.prevent="form.post(route('admin.categories.subcategories.store', { category: category.hashid }))"
