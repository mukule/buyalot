...
import { type BreadcrumbItem, type DocumentType } from '@/types';
...
const title = 'Edit Verification Document';
        ...
        <div class="p-4">
            <div class="flex h-full flex-1 flex-col space-y-4 rounded-xl bg-white p-4 text-[color:var(--card-foreground)]">
                <div class="flex items-center justify-between">
                    ...
                    <Link href="/admin/document-types" class="text-sm text-[color:var(--primary)] hover:underline">← Back</Link>
                </div>

                <hr class="my-1 border-[color:var(--border)]" />

                <form @submit.prevent="form.put(`/admin/document-types/${props.documentType.hashid}`)" class="mt-2 space-y-4 px-4">
                    <!-- Document Type Name -->
                    <div>
                        <input
                            v-model="form.name"
                    ...
                    >
                        {{ form.processing ? 'Updating...' : 'Submit' }}
                    </button>
                </form>
            </div>
        </div>
