...
const pathBreadcrumb = computed(
    () =>
        state.categoryPath.map((id) => categories.find((c) => c.id === id)).filter(Boolean) as Array<{
            id: number;
            name: string;
...
const builtConditions = computed(() => {
    switch (state.scope) {
        ...
        case 'customers':
            ...
            if (state.customerTarget === 'by_order_count')
                return { applies_to: 'customers', scope: 'by_order_count', min_orders: state.minOrderCount };
            if (state.customerTarget === 'by_total_spend')
                return { applies_to: 'customers', scope: 'by_total_spend', min_total_spend: state.minTotalSpend };
        ...
        <div class="p-4">
            <div class="card flex flex-col gap-6 rounded-lg bg-white p-4 shadow-sm">
                ...
                <div class="flex items-center justify-between">
                    <h1 class="text-2xl font-semibold text-gray-800">Create Promotion</h1>
                    <a
                ...
                <div class="rounded-lg bg-white p-4 shadow">
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div v-if="state.scope === 'categories'" class="grid grid-cols-1 gap-4 md:col-span-2">
                            <div>
                            ...
                            <div v-if="state.selectedLeafCategoryId">
                                ...
                                <p class="mt-1 text-xs text-gray-500">
                                    Note: This discount will target the selected category. Use the Products scope to target specific products.
                                </p>
                            </div>
                        </div>

                        <!-- Products selection -->
                        <div v-if="state.scope === 'specific_products'" class="grid grid-cols-1 gap-4 md:col-span-2 md:grid-cols-2">
                            <div>
                                <label class="mb-1 block text-sm font-medium text-gray-700">Filter by Category (optional)</label>
                                <select v-model="state.selectedCategoryIds" multiple class="h-40 w-full rounded border px-3 py-2">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    class="mb-2 w-full rounded border px-3 py-2"
                                />
                                <select v-model="state.selectedProductIds" multiple class="h-40 w-full rounded border px-3 py-2">
                        ...
                        <!-- Variants selection -->
                        <div v-if="state.scope === 'variants'" class="grid grid-cols-1 gap-4 md:col-span-2 md:grid-cols-3">
                            <div>
                                <label class="mb-1 block text-sm font-medium text-gray-700">Filter by Category (optional)</label>
                                <select v-model="state.selectedCategoryIds" multiple class="h-40 w-full rounded border px-3 py-2">
                                    <option v-for="c in categories" :key="c.id" :value="c.id">{{ c.name }}</option>
                            ...
                            <div>
                                ...
                                <input
                                    v-model="state.variantSearch"
                                    type="text"
                                    placeholder="Search variants..."
                                    class="mb-2 w-full rounded border px-3 py-2"
                                />
                                <select v-model="state.selectedVariantIds" multiple class="h-40 w-full rounded border px-3 py-2">
                                    <option v-for="v in filteredVariants" :key="v.id" :value="v.id">
                                        {{ v.display_name || v.sku || 'Variant #' + v.id }}
                                    </option>
                                </select>
                            </div>
                        </div>
                        ...
                        <!-- Customers selection -->
                        <div v-if="state.scope === 'customers'" class="grid grid-cols-1 gap-4 md:col-span-2">
                            <div>
                            ...
                            <div v-if="state.customerTarget === 'by_total_spend'" class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                <div>
                                    ...
                                    <input
                                        v-model.number="state.minTotalSpend"
                                        type="number"
                                        min="0"
                                        step="0.01"
                                        class="w-full rounded border px-3 py-2"
                            ...
                            <div v-if="state.customerTarget === 'specific'">
                                ...
                                <select v-model="state.selectedCustomerIds" multiple class="h-48 w-full rounded border px-3 py-2">
                                    <option v-for="c in customers" :key="c.id" :value="c.id">
                                        {{ [c.first_name || '', c.last_name || ''].join(' ').trim() || c.email }} ({{ c.email }})
                                    </option>
                                </select>
                            </div>
                        </div>
