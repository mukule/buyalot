...
const page = usePage();
const title = 'Create Product Status';
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-4">
            <div class="flex h-full flex-1 flex-col space-y-4 rounded-xl bg-white p-4 text-[color:var(--card-foreground)]">
                ...
                <form @submit.prevent="form.post('/admin/product-statuses')" class="mt-2 space-y-4 px-4">
                    <!-- Status Name -->
                    <div>
                        <input
                            v-model="form.name"
                    ...
                    <div>
                        <input
                            ...
                            id="color_class"
                            type="text"
                            placeholder="Tailwind color class (e.g., text-green-600)"
                            class="w-full rounded border border-[color:var(--border)] px-3 py-2 focus:ring-2 focus:ring-[color:var(--primary)] focus:outline-none"
                        />
