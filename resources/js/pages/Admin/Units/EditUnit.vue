...
import type { AppPageProps, BreadcrumbItem, Unit, UnitType } from '@/types';
import { Head, Link, useForm, usePage } from '@inertiajs/vue3';

const page = usePage<AppPageProps<{ unitType: UnitType; unit: Unit }>>();
const { unitType, unit } = page.props;

const title = `Edit Unit for ${unitType.name}`;
...
function submit() {
    form.put(
        route('admin.unit-types.units.update', {
            unit_type: unitType.hashid,
            unit: unit.hashid,
        }),
    );
}
