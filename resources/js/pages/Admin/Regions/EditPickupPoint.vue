...
const pickupPoint = page.props.pickupPoint;

const title = page.props.title || `Edit Pickup Point: ${pickupPoint.name}`;
const basePath = `/admin/regions/${region.hashid}/pickup-points/${pickupPoint.id}`;
...
// Form prefilled with pickup point data
const form = useForm({
    name: pickupPoint.name,
    description: pickupPoint.description || '',
    address: pickupPoint.address,
    contact_phone: pickupPoint.contact_phone || '',
    contact_email: pickupPoint.contact_email || '',
    active: pickupPoint.active == 1 ? true : false, // ✅ Bind active properly
});
...
// Computed property for map iframe (reactive)
const mapUrl = computed(() => {
    if (!form.address) return '';
    try {
        const url = new URL(form.address);
        if (url.hostname.includes('google.com') && url.pathname.includes('/maps')) {
            // Convert to embeddable URL
            if (form.address.includes('/maps/embed')) return form.address;
            return form.address.replace('/maps/', '/maps/embed/');
        }
    } catch {
        // Invalid URL
        return '';
        ...
        <div class="p-4">
            <div class="flex flex-col space-y-4 rounded-xl bg-white p-4 text-[color:var(--card-foreground)] shadow-sm">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <router-link :href="`/admin/regions/${region.hashid}`" class="cursor-pointer text-sm text-[color:var(--primary)] hover:underline">
                        ← Back
                    </router-link>
                </div>
                ...
                <form @submit.prevent="submitPickupPoint" class="space-y-4">
                    ...
                    <!-- Address -->
                    <div>
                        <label for="address" class="mb-1 block text-sm font-medium text-gray-700">Address / Google Maps URL</label>
                        <input
                    ...
                    <div v-if="mapUrl" class="mt-4">
                        <label class="mb-1 block text-sm font-medium text-gray-700">Map Preview</label>
                    ...
                    <button
                        ...
                        class="cursor-pointer rounded bg-[color:var(--primary)] px-4 py-2 text-white transition-colors duration-200 hover:bg-[color:var(--secondary)]"
                    >
                        {{ form.processing ? 'Updating...' : 'Update Pickup Point' }}
                    </button>
                </form>
            </div>
        </div>
