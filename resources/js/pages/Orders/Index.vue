...
<template>
    ...
    <div class="mx-auto max-w-6xl p-6">
        ...
        <div v-if="orders.length === 0" class="text-gray-500">You don’t have any orders yet.</div>

        <div v-else class="overflow-x-auto rounded-lg bg-white shadow">
            <table class="min-w-full border-collapse text-left text-sm">
                <thead class="bg-gray-100">
                    <tr>
                        <th class="border-b px-4 py-2">Order Code</th>
                ...
                <tbody>
                    <tr v-for="order in orders" :key="order.id" class="border-t">
                        ...
                        <td class="px-4 py-2">
                            <ul>
                                <li v-for="item in order.order_items ?? []" :key="item.id" class="text-gray-700">
                                    {{ item.product?.name ?? 'Unknown Product' }} (x{{ item.quantity }})
                                </li>
                            </ul>
                        </td>
                        <td class="px-4 py-2 font-semibold">{{ order.currency }} {{ order.total_amount.toFixed(2) }}</td>
                        <td class="px-4 py-2">
                            <span
                                :class="[
                        ...
                        <td class="px-4 py-2">
                            <Link :href="`/orders/${order.id}`" class="text-blue-600 hover:underline"> View </Link>
                        </td>
                    </tr>
                </tbody>
            </table>
